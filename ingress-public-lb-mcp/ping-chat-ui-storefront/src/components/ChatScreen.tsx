...
import { invokePingStoreAgent } from '../api/agent';
...
export default function ChatScreen() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      // Pass the user's AIC access token — the agent backend will perform
      // token exchange (RFC 8693) and call MCP tools on the user's behalf
      const token = getAccessToken();
      if (!token) {
        addMessage('error', 'Session expired. Please log in again.');
        return;
      }
      const reply = await invokePingStoreAgent(text, token);
