...
const contextKeyEmail contextKey = "user-email"

// registerTools adds all Stripe MCP tools to the server.
func registerTools(s *server.MCPServer) {
	s.AddTool(listProductsTool())
	s.AddTool(getProductTool())
	s.AddTool(getCustomerTool())
	s.AddTool(createPaymentIntentTool())
}
...
func getCustomerTool() (mcp.Tool, server.ToolHandlerFunc) {
	...
	handler := func(ctx context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		customerEmail, ok := ctx.Value(contextKeyEmail).(string)
		if !ok || customerEmail == "" {
...
func createPaymentIntentTool() (mcp.Tool, server.ToolHandlerFunc) {
	tool := mcp.NewTool("create_payment_intent",
		mcp.WithDescription("Purchase a product on behalf of the authenticated user using their saved payment method on file. Stripe will send a receipt to their email. IMPORTANT: Always call get_customer first to retrieve and confirm the card on file with the user before calling this tool."),
		mcp.WithString("product_id",
			mcp.Required(),
